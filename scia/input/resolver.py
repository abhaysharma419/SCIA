...
def resolve_input(
    ...
    if before_format == 'json' and after_format == 'json':
    ...
    elif (before_format == 'sql' and after_format == 'json') or \
         (before_format == 'json' and after_format == 'sql') or \
         (before_format == 'sql' and after_format == 'sql'):
        input_type = InputType.SQL
        
    elif before_format == 'database' and after_format == 'database':
        ...
        # Mixed database and file mode
        if not warehouse:
            raise InputResolutionError(
                "Comparing live database to file requires --warehouse parameter"
            )
        input_type = InputType.DATABASE
