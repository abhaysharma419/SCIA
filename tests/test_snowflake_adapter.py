...
def test_snowflake_connection_success(mock_snowflake_conn):
...
def test_snowflake_fetch_schema_success(mock_snowflake_conn):
...
def test_snowflake_fetch_schema_empty(mock_snowflake_conn):
    mock_cursor = MagicMock()
    mock_snowflake_conn.cursor.return_value = mock_cursor
    mock_cursor.fetchall.return_value = []
...
def test_snowflake_fetch_views_success(mock_snowflake_conn):
    mock_cursor = MagicMock()
    mock_snowflake_conn.cursor.return_value = mock_cursor
    mock_cursor.fetchall.return_value = [
...
def test_snowflake_fetch_foreign_keys_stub(mock_snowflake_conn):
    inspector = SnowflakeInspector({"user": "test"})
