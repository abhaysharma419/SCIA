...
def test_rule_join_key_changed_applies(schema_diff_factory, column_factory):
    ...
    diff = schema_diff_factory(column_changes=[
        ColumnDiff(schema_name="S", table_name="T", column_name="USER_ID", change_type="REMOVED", before=col)
    ])
...
def test_rule_grain_change_applies(schema_diff_factory, column_factory):
    ...
    diff = schema_diff_factory(column_changes=[
        ColumnDiff(schema_name="S", table_name="T", column_name="REGION", change_type="REMOVED", before=col)
    ])
    mock_metadata = MagicMock()
...
def test_rule_potential_breakage_type_change(schema_diff_factory, column_factory):
    ...
    diff = schema_diff_factory(column_changes=[
        ColumnDiff(schema_name="S", table_name="T", column_name="REVENUE", 
